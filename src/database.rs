...
pub async fn rest_edit_title(rest_num: i32, new_str: String) -> bool {
   execute("UPDATE restaurants SET title = $1::VARCHAR(100) WHERE rest_num=$2::INTEGER", &[&new_str, &rest_num]).await
}

pub async fn rest_edit_info(rest_num: i32, new_str: String) -> bool {
   execute("UPDATE restaurants SET info = $1::VARCHAR(255) WHERE rest_num=$2::INTEGER", &[&new_str, &rest_num]).await
}

pub async fn rest_toggle(rest_num: i32) -> bool {
   execute("UPDATE restaurants SET active = NOT active WHERE rest_num=$1::INTEGER", &[&rest_num]).await
}

// Изменение фото ресторана
pub async fn rest_edit_image(rest_num: i32, image_id: &String) -> bool {
   execute("UPDATE restaurants SET image_id = $1::VARCHAR(255) WHERE rest_num=$2::INTEGER", &[&image_id, &rest_num]).await
}

// Изменяет владельца ресторана
pub async fn transfer_ownership(rest_num: i32, new_user_id: i32) -> bool {
   execute("UPDATE restaurants SET user_id = $1::INTEGER WHERE rest_num=$2::INTEGER", &[&new_user_id, &rest_num]).await
}
...
pub async fn register_caterer(user_id: i32) -> bool {
   ...
   if execute("UPDATE restaurants SET enabled = TRUE WHERE user_id=$1::INTEGER", &[&user_id]).await {
      return true;
   }
...
pub async fn hold_caterer(user_id: i32) -> bool {
   execute("UPDATE restaurants SET enabled = FALSE, active = FALSE WHERE user_id=$1::INTEGER", &[&user_id]).await
}
...
pub async fn rest_group_edit_title(rest_num: i32, group_num: i32, new_str: String) -> bool {
   execute("UPDATE groups SET title = $1::VARCHAR(100) WHERE rest_num=$2::INTEGER AND group_num=$3::INTEGER", &[&new_str, &rest_num, &group_num]).await
}

// Изменяет описание группы
pub async fn rest_group_edit_info(rest_num: i32, group_num: i32, new_str: String) -> bool {
   execute("UPDATE groups SET info = $1::VARCHAR(255) WHERE rest_num=$2::INTEGER AND group_num=$3::INTEGER", &[&new_str, &rest_num, &group_num]).await
}

// Переключает доступность группы
pub async fn rest_group_toggle(rest_num: i32, group_num: i32) -> bool {
   execute("UPDATE groups SET active = NOT active WHERE rest_num=$1::INTEGER AND group_num=$2::INTEGER", &[&rest_num, &group_num]).await
}

// Изменяет категорию группы
pub async fn rest_group_edit_category(rest_num: i32, group_num: i32, new_cat : i32) -> bool {
   execute("UPDATE groups SET cat_id = $1::INTEGER WHERE rest_num=$2::INTEGER AND group_num=$3::INTEGER", &[&new_cat, &rest_num, &group_num]).await
}
...
pub async fn rest_group_edit_time(rest_num: i32, group_num: i32, opening_time: NaiveTime, closing_time: NaiveTime) -> bool {
   if execute("UPDATE groups SET opening_time = $1::TIME, closing_time = $2::TIME WHERE rest_num=$3::INTEGER AND group_num=$4::INTEGER", &[&opening_time, &closing_time, &rest_num, &group_num]).await {
...
pub async fn dish_list(by: DishesBy) -> Option<DishList> {
   ...
   let statement_text =  match by {
      ...
      DishesBy::Find(_) =>
         ...
         WHERE r.active = TRUE AND r.enabled = TRUE AND g.active = TRUE AND UPPER(d.title) like UPPER($1::VARCHAR(100)) ORDER BY r.rest_num, g.group_num",
   };
