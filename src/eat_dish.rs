...
async fn inline_data(cat_id: i32, rest_num: i32, group_num: i32) -> InlineData {
   ...
   let (text, markup) = match db::group(rest_num, group_num).await {
      None => {
         // Такая ситуация не должна возникнуть
         // Кнопка назад
         let buttons = vec![InlineKeyboardButton::callback(String::from("В меню"), format!("rrg{}", db::make_key_3_int(rest_num, cat_id, 0)))];
         // Формируем меню
      ...
      Some(group) => {
         ...
         let markup = match db::dish_list(db::DishesBy::Active(rest_num, group_num)).await {
            None => {
               ...
               let buttons = vec![InlineKeyboardButton::callback(String::from("В меню"), format!("rca{}", db::make_key_3_int(cat_id, 0, 0)))];
               let markup = InlineKeyboardMarkup::default()
               .append_row(buttons);
               markup
            }
            Some(dishes) => {
               ...
               // Кнопка назад
               let button_back = InlineKeyboardButton::callback(String::from("В меню"), format!("rrg{}", db::make_key_3_int(rest_num, cat_id, 0)));
